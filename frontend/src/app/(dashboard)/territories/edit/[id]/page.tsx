...
export default function EditTerritoryPage() {
  ...
  useEffect(() => {
    api.get(`/territories/${params.id}`)
      .then(res => setInitialData(res))
      .catch(() => router.push('/territories'))
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center space-x-4 space-x-reverse mb-6">
        <Link href="/territories" className="p-2 bg-white dark:bg-slate-900 rounded-full shadow-sm hover:bg-slate-50 dark:bg-slate-800/50 transition border border-slate-200 dark:border-slate-700">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
        <TerritoryForm 
          ...
          onCancel={() => router.push('/territories')}
        />
      </div>
    </div>
  );
