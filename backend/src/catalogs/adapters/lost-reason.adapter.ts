...
export class LostReasonAdapter implements CatalogAdapter<any> {
  ...
  async findOne(prisma: PrismaService, id: string): Promise<any | null> {
    return prisma.lostReason.findUnique({ where: { id } });
  }

  validateCreate(data: any): void {
    const dto = plainToInstance(LostReasonDto, data, { excludeExtraneousValues: false });
  ...
  validateUpdate(data: any): void {
    const dto = plainToInstance(UpdateLostReasonDto, data, { excludeExtraneousValues: false });
    const errors = validateSync(dto);
    if (errors.length > 0) throw new BadRequestException('Validation failed');
    const allowedKeys = ['code', 'nameEn', 'nameFa', 'description', 'isActive', 'sortOrder'];
    for (const key of Object.keys(data)) {
  ...
  async create(prisma: PrismaService, data: any, userId: string): Promise<any> {
    ...
    const existing = await prisma.lostReason.findUnique({ where: { code: data.code } });
  ...
  async update(prisma: PrismaService, id: string, data: any, userId: string): Promise<any> {
    ...
    if (data.code && data.code !== existing.code) {
      const dupe = await prisma.lostReason.findUnique({ where: { code: data.code } });
  ...
  async softDelete(prisma: PrismaService, id: string, userId: string): Promise<void> {
    const existing = await prisma.lostReason.findUnique({
      where: { id },
      include: { _count: { select: { opportunities: true } } }
    });
    if (!existing) throw new NotFoundException('Record not found');
    ...
    await prisma.lostReason.update({ where: { id }, data: { isActive: false } });
  }
}
