...
class UpdatePresentationMethodDto {
...
export class PresentationMethodAdapter implements CatalogAdapter<any> {
  ...
  async findAll(prisma: PrismaService, params: any): Promise<PaginatedResult<any>> {
    ...
    const [total, data] = await Promise.all([
      prisma.presentationMethod.count({ where }),
      prisma.presentationMethod.findMany({
        where,
        orderBy,
        skip,
        take: pageSize
  ...
  async findOne(prisma: PrismaService, id: string): Promise<any | null> {
    return prisma.presentationMethod.findUnique({ where: { id } });
  }

  validateCreate(data: any): void {
    const dto = plainToInstance(PresentationMethodDto, data, { excludeExtraneousValues: false });
  ...
  validateUpdate(data: any): void {
    const dto = plainToInstance(UpdatePresentationMethodDto, data, { excludeExtraneousValues: false });
    const errors = validateSync(dto);
    if (errors.length > 0) throw new BadRequestException('Validation failed');
    const allowedKeys = ['code', 'nameEn', 'nameFa', 'description', 'isActive', 'sortOrder'];
    for (const key of Object.keys(data)) {
  ...
  async create(prisma: PrismaService, data: any, userId: string): Promise<any> {
    ...
    const existing = await prisma.presentationMethod.findUnique({ where: { code: data.code } });
    if (existing) throw new ConflictException('Code already exists');
    return prisma.presentationMethod.create({ data });
  }

  async update(prisma: PrismaService, id: string, data: any, userId: string): Promise<any> {
    this.validateUpdate(data);
    const existing = await prisma.presentationMethod.findUnique({ where: { id } });
    ...
    if (data.code && data.code !== existing.code) {
      const dupe = await prisma.presentationMethod.findUnique({ where: { code: data.code } });
      if (dupe) throw new ConflictException('Code already exists');
    }

    return prisma.presentationMethod.update({ where: { id }, data });
  }

  async softDelete(prisma: PrismaService, id: string, userId: string): Promise<void> {
    const existing = await prisma.presentationMethod.findUnique({
      where: { id }
    });
    if (!existing) throw new NotFoundException('Record not found');
    if (existing.isSystem) throw new ForbiddenException('System records cannot be deleted');

    // We don't have usage checks for PresentationMethod since it's a loosely coupled string in Presentation table.
    // In a real app we'd query the Presentation table for usage.

    await prisma.presentationMethod.update({ where: { id }, data: { isActive: false } });
  }
}
